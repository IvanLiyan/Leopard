// Test enabling this rule once we have tests for these validators
...
/* Toolkit */
import { DatetimeInput } from "@schema";
...
export class CharacterLength extends Validator {
  ...
  // function must be async due to parent Validator's typing
...
// TODO [lliepert]: bring back validator with re-written API call
  ...
  async validateText(text = ""): Promise<ValidationResponse> {
    ...
    try {
      ...
      if (countryCode == "US") {
        ...
        if (stateKey) {
          // @ts-ignore key does exist based on prior check, legacy code
          phoneCountryCode = usSpecialStateMap[stateKey];
        }
      }
      ...
      if (!isValid) {
        // legacy code - logic is valid in this case
...
export class TaxNumberValidator extends Validator {
  ...
  getRequirements(): ReadonlyArray<Validator> {
    ...
    let pattern: string | null | undefined = null;

    if (stateCode == null) {
      // @ts-ignore legacy code, not type safe (countryCode might not be a CountryCode)
      pattern = this.CountryNumberMasks[countryCode];
    } else {
