...
import { WsssUnfulfilledReasons } from "@schema";
import { WssUnfulfilledReasonLabel } from "@performance/migrated/toolkit/order-metrics";
import { StyleSheet } from "aphrodite";
import { observer } from "mobx-react";
import React, { useMemo, useState } from "react";

type Props = BaseProps & {
  readonly options?: ReadonlyArray<WsssUnfulfilledReasons> | null;
  readonly selected?: ReadonlyArray<WsssUnfulfilledReasons> | null;
  readonly onConfirm?: (reasons: ReadonlyArray<WsssUnfulfilledReasons>) => void;
};

const UnfulfilledReasonFilter: React.FC<Props> = (props) => {
  const {
    className,
    style,
    options,
      ...
      <CheckboxGroupField
        ...
        options={options
          .map((reason) => {
            return {
              value: reason,
              title: WssUnfulfilledReasonLabel[reason],
              key: reason,
            };
          })
          .sort((a, b) => a.title.localeCompare(b.title, locale))}
        selected={unconfirmedSelected}
        onChecked={(option: { readonly value: WsssUnfulfilledReasons }) => {
...
export default observer(UnfulfilledReasonFilter);
