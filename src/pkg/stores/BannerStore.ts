...
export default class BannerStore {
  ...
  async fetchBannerItems(): Promise<void> {
    ...
    void this.logBannerImpressions(this.bannerItems);
  }

  @action
  fetchBannerFlags = async (): Promise<void> => {
    const { client } = this.apolloStore;
  ...
  async fetchCanShowNewNavBanner(): Promise<void> {
    const optedSelfInState = this.initialData?.currentUser?.uiState.bool;
    const bucket = await this.experimentStore.getBucketForMerchant(
      "md_new_nav_phase_2",
    );
  ...
  async fetchCanShowPlpBanner(): Promise<void> {
    const canSeePlp = await this.experimentStore.getDeciderKeyDecision(
      "product_listing_plan_fe",
    );
    this.canShowPlpBanner = canSeePlp;
  }

  async fetchCanShowTosBanner(): Promise<void> {
    const canShowTosBanner = await this.experimentStore.getDeciderKeyDecision(
      "tos_update_2021",
    );
  ...
  get bannerItems(): ReadonlyArray<HomeBannerItem> {
    ...
    const { textBlack, primary, surfaceLightest, primaryLight } =
      this.themeStore.currentAppTheme();
    if (initialData == null) {
      return [];
    }
    ...
    return [
      ...
      },
      {
        ...
        component:
          "CustomerServiceProgramBanner" as HomeBannerItem["component"],
        shouldShow: () => true,
        background: surfaceLightest,
        componentProps: {
          logParams: {
      ...
      },
      {
        ...
        component:
          "PriceDropMarketingCampaignBanner" as HomeBannerItem["component"],
      ...
      },
      {
        ...
        component:
          "WeeklyDisbRequiredUpdateBanner" as HomeBannerItem["component"],
        shouldShow: () =>
          !paymentDetails.fullyEnrolledInPaymentCycle &&
          paymentDetails.paymentCycle === "WEEKLY",
        background: surfaceLightest,
  ...
  @action
  logBannerImpressions(banners: ReadonlyArray<HomeBannerItem>): void {
    banners.forEach((banner) => {
      ...
      void log(BannerImpressionLogTable, logParams);
    });
  }

  @action
  fetchProductBoostBannerItems = async (): Promise<void> => {
    ...
    const { loggedInMerchantUser } = this.userStore;
    const { surfaceLightest } = this.themeStore.currentAppTheme();
  ...
  @action
  fetchCollectionBoostBannerItems = async (): Promise<void> => {
  ...
  @action
  fetchSellerProfileItems = async (): Promise<void> => {
    const { surfaceLightest } = this.themeStore.currentAppTheme();
  ...
  get merchantId(): string | null | undefined {
    const { merchantId } = this.userStore;
    return merchantId;
  }
}
