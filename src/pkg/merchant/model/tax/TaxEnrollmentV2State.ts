...
export default class TaxEnrollmentV2State {
  ...
  get currentCountryCode(): string | null | undefined {
    ...
    const { countryCode } = navStore.pathParams("/tax/v2-enroll/:countryCode");

    if (countryCode) {
      return countryCode;
    }

    return navStore.pathParams("/tax/v2-enroll/:countryCode/:substep")
      .countryCode;
  }
  ...
  get currentIndex(): number {
    ...
    return findIndex(this.countrySteps, { countryCode } as any);
  }
  ...
  get previousStepPath(): string | null | undefined {
    ...
    if (navStore.currentPath === "/tax/v2-enroll/review") {
  ...
  get nextStepPath(): string | null | undefined {
    ...
    if (navStore.currentPath === "/tax/v2-enroll") {
    ...
    // UK VAT direct link, we don't want to go back to country selection
    if (navStore.currentPath === "/tax/v2-enroll/GB") {
  ...
  setCountrySelected(countryCode: CountryCode, selected: boolean) {
    ...
    if (selected) {
      ...
      if (countryCode !== "US") {
        const index = findIndex(this.pendingTaxInfos, {
          countryCode,
  ...
  getCountryLevelSettings = (
    ...
    return find(this.pendingTaxInfos, {
      countryCode,
      authorityLevel: "COUNTRY",
    } as any);
  };
  ...
  setLocalAuthoritySelected = (
    ...
    if (selected) {
      const index = findIndex(this.pendingTaxInfos, params);
      if (index >= 0) {
        return;
      }
      ...
      let index = findIndex(this.pendingTaxInfos, params);
      if (index < 0) {
        return;
      }
    ...
    this.setStateSelected({
      ...
      selected: this.currentStates(params.countryCode).includes(
        params.stateCode
      ),
    });
  };
  ...
  isLocalAuthoritySelected = (params: {
    ...
    return find(this.pendingTaxInfos, params) != null;
  };
  ...
  hasStartedSettingsForCountry = (countryCode: string): boolean => {
    ...
    return (
      ...
      (countryInfos.length === 1 && !isEmpty(countryInfos[0].taxNumber))
    );
  };
  ...
  submit = async (doNotRedirect?: boolean) => {
    ...
    const updatedCountryCodesParam = uniq(updatedCountryCodes).join(",");
