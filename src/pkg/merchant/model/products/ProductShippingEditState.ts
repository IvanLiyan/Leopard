...
type WishPostEstimatedShippingSchema = Pick<
  WishPostShippingSchema,
  "country" | "maxPrice" | "minPrice"
>;
...
> & {
  ...
  readonly shipping: {
    ...
    readonly calculatedShippingEnabled: null | undefined | Scalars["Boolean"];
    readonly wishpostEstimatedShipping:
      | null
      | undefined
      | ReadonlyArray<WishPostEstimatedShippingSchema>;
  };
};
...
export default class ProductShippingEditState {
  ...
  // TODO: To deprecate after allow_enable_disable_calculated_shipping flag is removed
  // https://jira.wish.site/browse/MKL-54217
  @observable
  storeCountries: ReadonlyArray<PickedShippingSettingsSchema>;

  // TODO: Rename to storeCountries after allow_enable_disable_calculated_shipping flag is removed
  // https://jira.wish.site/browse/MKL-54217
  @observable
  storeCountriesWithUnityCountries: ReadonlyArray<PickedShippingSettingsSchema>;
  ...
  // TODO: Rename to warehouseConfiguredCountries after allow_enable_disable_calculated_shipping flag is removed
  // https://jira.wish.site/browse/MKL-54217
  @observable
  warehouseConfiguredCountriesWithUnityCountries: ReadonlyArray<PickedCountryShippingSchema>;

  @observable
  accountLevelEnabledCountryCodes: ReadonlyArray<CountryCode>;
  ...
  @observable
  private wishpostEstimatedPrices: Map<
    CountryCode,
    WishPostEstimatedShippingSchema
  > = new Map();
  ...
  constructor(args: {
    ...
    readonly isMerchantInCalculatedShippingBeta: boolean;
    ...
    this.isMerchantInCalculatedShippingBeta =
      isMerchantInCalculatedShippingBeta;
    ...
    // TODO: Overrule domestic shipping option based "Unity" enabled status
    // If merchant is in user gate.
    const isCalculatedShippingEnabled =
      initialState.shipping?.calculatedShippingEnabled;
    if (
      isCalculatedShippingEnabled != null &&
      isMerchantInCalculatedShippingBeta
    ) {
      this.domesticShippingEnabledByWarehouseId.set(
        warehouseId,
        isCalculatedShippingEnabled
      );
    }

    for (const estimatedShipping of initialState.shipping
      ?.wishpostEstimatedShipping || []) {
      this.wishpostEstimatedPrices.set(
        estimatedShipping.country.code,
        estimatedShipping
      );
    }

    const accountLevelEnabledCountryCodes: CountryCode[] = [];
    for (const country of storeCountries || []) {
      accountLevelEnabledCountryCodes.push(country.country.code);
    }
    this.accountLevelEnabledCountryCodes = accountLevelEnabledCountryCodes;
        ...
        if (storeCountryCodes.includes(countryCode)) {
          if (countryShipping.enabled) {
            allCountryShippingSet =
              allCountryShippingSet && countryShipping.price?.amount != 0;
          }
    ...
    // TODO: Rename this to storeCountries after allow_enable_disable_calculated_shipping flag is removed
    // https://jira.wish.site/browse/MKL-54217
    this.storeCountriesWithUnityCountries = sortedStoreCountries;

    // TODO: Rename subsequent properties to original after allow_enable_disable_calculated_shipping flag is removed
    // https://jira.wish.site/browse/MKL-54217
    const storeCountriesWithUnityCountriesCodes =
      this.storeCountriesWithUnityCountries.map(
        (country) => country.country.code
      );
    const warehouseConfiguredCountriesWithUnityCountries = [];
    for (const warehouseCountryShipping of warehouseCountryShippingSettings ||
      []) {
      for (const countryShipping of warehouseCountryShipping.countryShipping ||
        []) {
        ...
        if (storeCountriesWithUnityCountriesCodes.includes(countryCode)) {
          if (countryShipping.enabled) {
            allCountryShippingSet =
              allCountryShippingSet && countryShipping.price?.amount != 0;
          }
          ...
          this.countryShippingStates.set(countryCode, countryShippingState);
          warehouseConfiguredCountriesWithUnityCountries.push(countryShipping);
        }
      }
    }
    this.warehouseConfiguredCountriesWithUnityCountries = Array.from(
  ...
  get showAdvancedSection(): boolean {
    ...
    return (
      isCnMerchant &&
      isStandardWarehouse &&
      (isUnityEnabled || isMerchantInCalculatedShippingBeta) &&
      !isUnityBlacklisted
    );
  }

  // For legacy Unity - will be deprecated as use case in AdvancedLogisticsSection will be removed
  // https://jira.wish.site/browse/MKL-54646
  @computed
  ...
  // For legacy Unity - will be deprecated in favour of showAdvancedSection for CSP
  // https://jira.wish.site/browse/MKL-54646
  @computed
  ...
  @computed
  get hasUnityCountriesEnabledAccountLevel(): boolean {
    const { accountLevelEnabledCountryCodes, unityCountries } = this;
    const unityCountryCodes = unityCountries.map((country) => country.code);
    return accountLevelEnabledCountryCodes.some((code) =>
      unityCountryCodes.includes(code)
    );
  }
  ...
  get showDefaultShippingConfirmation(): boolean {
    ...
    const initialDefaultShippingPrices =
      this.initialState.shipping?.defaultShipping;

    if (
      defaultShippingPrice !==
      initialDefaultShippingPrices?.find(
        (price) => price.warehouseId === warehouseId
      )?.price?.amount
    ) {
      return true;
    }
  ...
  // For legacy Unity - will be deprecated replaced by showCalculatedShippingEUComplianceBanners
  // https://jira.wish.site/browse/MKL-54646
  @computed
  ...
  @computed
  get showCalculatedShippingEUComplianceBanners(): boolean {
    return this.showAdvancedSection && this.showEUComplianceBanners;
  }
  ...
  // This property is a dummy property, it always returns true.
  // Will need to be implemented property by comparing self to initialState values.
  // https://jira.wish.site/browse/MKL-54914
  @computed
  ...
  // Note: domestic shipping == calculated shipping
  ...
  getCountryEstimatedShipping(
    country: PickedCountry
  ): WishPostEstimatedShippingSchema | undefined {
    return this.wishpostEstimatedPrices.get(country.code);
  }
  ...
  async submit() {
    const {
      name,
      asInput: inputOld,
      asInputWithEstimatedShipping: inputNew,
      isMerchantInCalculatedShippingBeta,
    } = this;
    const toastStore = ToastStore.instance();
    ...
    const input = isMerchantInCalculatedShippingBeta ? inputNew : inputOld;
  ...
  private get asInput(): ProductUpsertInput {
    ...
    // If estimated shipping, then allow unity countries to be enabled when domestic shipping isnt
    const unityDisabledCountryInputList =
      this.showAdvancedSection && domesticShippingEnabled === false
  ...
  @computed
  private get asInputWithEstimatedShipping(): ProductUpsertInput {
    const {
      productId,
      primaryCurrency,
      countryShippingStates,
