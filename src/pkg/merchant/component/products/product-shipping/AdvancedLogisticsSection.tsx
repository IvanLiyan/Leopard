    ...
    return (
      <>
        <HorizontalField
          titleWidth={400}
          titleAlign={"start"}
          title={i`Ship to all countries included in the unification initiative`}
          className={css(styles.verticalMargin)}
        >
          <Switch
  ...
  return (
    <div className={css(styles.root)}>
      ...
      {editState.showUnityEUComplianceBanners && (
        <div className={css(styles.form)}>
          {editState.isAfterEUComplianceDate ? (
            ...
            <Alert
              text={
                i`This product is identified by Wish as likely being subject ` +
                i`to the upcoming EU regulation. As such, impressions and sales ` +
                i`in all EU countries (including the EU countries listed below ` +
                i`that are supported by Wish's unification initiative) for this product ` +
                i`will be blocked if a Responsible Person has not been linked by July 9, 2021.`
              }
              sentiment="warning"
            />
          )}
      ...
      <div className={css(styles.greyBackground)}>
        Destination countries included in Wish’s shipping unification initiative
      </div>
