...
const CustomizeShippingSection: React.FC<Props> = ({ editState }: Props) => {
  ...
  if (editState.isPromoted) {
    alerts.push({
      text:
        i`This product is currently being promoted to receive ` +
        i`additional impressions in the Wish app. In the interest of ` +
        i`providing your customers with the best shopping experience ` +
        i`and avoiding abrupt changes in the shipping prices, ` +
        i`the Shipping Price cannot be increased. `,
      sentiment: "info",
      link: {
        text: i`Learn more`,
        url: zendeskURL("205212507"),
      },
      title: i`Product under Promotion`,
    });
  }
      ...
      {editState.showAdvancedSection && (
        <Markdown
          ...
          text={
            ...
            i`country(ies)/region(s) added. To add new countries/regions you ship ` +
