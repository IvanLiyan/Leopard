...
type FBSShippingPlanConfirmTableProps = BaseProps & {
...
@observer
class FBSShippingPlanConfirmTable extends Component<
  FBSShippingPlanConfirmTableProps
> {
  itemsInOnePage = 5;
  ...
  renderQuantity(row: Product) {
    const { variationWarehouseQuantity, warehouses } = this.props;
    let quantitySum = 0;
    warehouses.forEach((wh) => {
      quantitySum +=
        variationWarehouseQuantity
          .get(row.variation_id)
          ?.get(wh.warehouse_code || "") || 0;
    });
    return <section>{quantitySum}</section>;
  }
...
export default FBSShippingPlanConfirmTable;
