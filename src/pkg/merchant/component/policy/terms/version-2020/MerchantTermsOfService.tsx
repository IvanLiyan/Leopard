...
export default observer((props: BaseProps) => {
  ...
  const merchantPolicyLink = "/policy/home";
  const apiTOSLink = "/api-partner-terms-of-service";
  const taxPolicyLink = "/tax/policy";
  const termsOfUseLink = wishURL("/terms");
  const privacyPolicyLink = wishURL("/privacy_policy");
  const eeaLink = "/merchant-payout-service-terms-eea";
  const merchantSupportEmail = "mailto:merchant_support@wish.com";
  const feesAndPaymentsLink = "/policy/fees_and_payments";
  const productBoostLink = "/product-boost";
  const jamsadrLink = "http://www.jamsadr.com";
  const jamsadrRulesSlLink =
    "http://www.jamsadr.com/rules-streamlined-arbitration/";
  const jamsadrRulesCompLink =
  ...
  return (
    <div className={css(className, style)}>
      <p>
        <strong>
          These Merchant Terms of Service and Agreement were updated on July 3,
          2020. If you register on or after July 3, 2020, your use of the
          Services is governed by the terms below. If you registered before July
          3, 2020, your use of the Services will be governed by the terms below
          as of July 10, 2020.
        </strong>
        &nbsp;
        <Link openInNewTab href="/terms-of-service?version=3">
      ...
      <li>
        <Link openInNewTab href={apiTOSLink}>
          Wish API Terms of Service
        </Link>
      </li>
      <li>
        <Link openInNewTab href={merchantPolicyLink}>
          Merchant Policies
        </Link>
        &nbsp;
        <span>(including all subsections incorporated therein)</span>
      </li>
      <li>
        <Link openInNewTab href={taxPolicyLink}>
          Tax Policy
        </Link>
      </li>
      <li>
        <Link openInNewTab href={feesAndPaymentsLink}>
      ...
      <li>
        <Link openInNewTab href={privacyPolicyLink}>
          Privacy Policy
        </Link>
      </li>
      <li>
      ...
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`As a legal person, you represent, warrant, promise and guarantee that ` +
            i`during the period of registration and agreement: (a) you are legally ` +
            i`established in accordance with applicable law, validly existing and in ` +
            i`good operation; (b) you have all the necessary legal qualifications, ` +
            i`rights, capabilities and authorities to sign this agreement, fulfill ` +
            i`duties accordingly and grant rights, licensing and authority required ` +
            i`by this agreement, and have the permissions, approvals and licenses ` +
            i`required by your business and the sale of the items in the relevant ` +
            i`countries; (c) you and your affiliates will comply with all laws to ` +
            i`fulfill your rights and duties in this agreement; (d) when offering ` +
            i`any items for sale through the Services, you are in full compliance ` +
            i`with all legislation, statutes, regulations and other enactments ` +
            i`having the force of law and all industry codes, policies or guidelines ` +
            i`and any applicable direction, statement of practice, policy, rule or ` +
            i`order given by a regulator which apply from time to time in the ` +
            i`country from which or to which the items are sold and/or offered ` +
            i`(“Applicable Laws and Regulations”); (e) you shall maintain such ` +
            i`records as are necessary pursuant to such Applicable Laws and ` +
            i`Regulations and shall promptly on request make them available for ` +
            i`inspection by any relevant authority that is entitled to inspect them; ` +
            i`(f) you shall monitor any changes in the Applicable Laws and ` +
            i`Regulations which may impact the sale of the items through the ` +
            i`Services; (g) you shall directly notify Wish by email and in writing ` +
            i`of any investigation and potential claim that are instigated by any ` +
            i`regulator in relation to the items offered through the Services; (h) ` +
            i`you shall promptly remove any and all offerings of items from the ` +
            i`Services whenever these infringe the Applicable Laws and Regulations, ` +
            i`become otherwise prohibited in the relevant countries, and/or when ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`these are included in (an updated version of) the [Merchant Policies](${merchantPolicyLink}) ` +
            i`and (i) you and your financial institution(s) are not subject to ` +
            i`sanctions or otherwise designated on any list of prohibited or ` +
            i`restricted parties or owned or controlled by such a party, including ` +
            ...
            i`and the US Department of Commerce’s Entity List), the European Union` +
            i`or its member states, or other applicable government authority.`
          }
        />
      </p>
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`Additionally, should you register an account, make purchases or ` +
            i`otherwise use our Services in a capacity other than as a merchant, ` +
            i`seller or distributor (e.g., as a purchasing consumer and/or retail ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`customer), you agree to be bound by Wish’s [Terms of Use](${termsOfUseLink}) ` +
            i`and those Terms of Use shall govern such conduct. `
          }
        />
      </p>
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`If you are located in the European Economic Area or the United ` +
            i`Kingdom, in order to receive funds resulting from consumer purchases, ` +
            i`you must contract with ContextLogic B.V. If you are located in the ` +
            i`European Economic Area or the United Kingdom and receive funds ` +
            i`resulting from consumer purchases, you accept the ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`[Merchant Pay-out Service Terms](${eeaLink})  offered by ContextLogic B.V. `
          }
        />
      </p>
      <p>
      ...
      <p>
        <strong>
          If you have your place of establishment or residence in the European
          Union and offer goods or services to consumers located in the European
          Union (“EU-Merchant”), the following terms apply instead:
        </strong>
      </p>
      <p>
        <span>
          We will notify you of any proposed changes of our Terms on a durable
          medium, e.g., via our Merchant platform or email (“Notice”).
        </span>
      </p>
      <p>
        <span>
          We will not impose any retroactive changes to terms and conditions,
          except when they are required to respect a legal or regulatory
          obligation or when the retroactive changes are beneficial for you.
        </span>
      </p>
      <p>
        <span>
          The proposed changes shall not be implemented before the expiry of a
          notice period of a minimum of fifteen (15) days from the date on which
          we notify you about the proposed changes (“Notice Period”). You have
          the right to terminate this Agreement before the expiry of the notice
          period. Such termination shall take effect within fifteen (15) days
          from the receipt of the Notice, unless a shorter period applies to
          this Agreement.
        </span>
      </p>
      <p>
        <span>
          ...
          action, waive the Notice Period at any moment from the receipt of the
          Notice. During the Notice Period, submitting new goods or services in
          connection with the Wish Service shall be considered clear affirmative
          action to waive the Notice Period, except in cases where the
          reasonable and proportionate Notice Period is longer than fifteen (15)
          days because the changes to the Terms require you to make significant
          technical adjustments to your goods or services. In such cases, the
          Notice Period shall not be considered automatically to be waived where
          you submit new goods and services.
        </span>
      </p>
      <p>
      ...
      <p>
        <strong>
          If you are an EU-Merchant, regarding the Right to Refuse Service, the
          following terms apply instead:
        </strong>
      </p>
      <p>
        <span>
          Wish reserves the right, in Wish’s sole discretion, to cancel accounts
          and/or to refuse to offer the Services to you for the following
          reasons: (i) your account remains unconfirmed; (ii) your account
          remains inactive for a period of one-hundred-eighty (180) days; (iii)
          we determine that your account has been used to engage in deceptive,
          fraudulent, or illegal activity, or to substantially violate our
          policies or this Agreement; (iv) we determine that the performance of
          your obligations under this Agreement may result in returns, claims,
          disputes, violations of our terms or policies, or cause any other
          risks to Wish, its users or other third parties; or (v) we determine,
          suspect, or are informed that you are selling goods or engaging in
          acts in violation of the prohibited activities defined below under
          Section 5 of this Agreement (including, without limitation, selling
          goods that are counterfeit, illegal, or violate third-party rights).
        </span>
      </p>
      <p>
        ...
        <Markdown
          openLinksInNewTab
          text={
            i`You have the right to close your account. If you choose to close your ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`account, please contact [merchant_support@wish.com](${merchantSupportEmail}). ` +
            i`You will then receive a confirmation once your account has been ` +
            i`closed. If Wish is unable to close your account at this time, you also ` +
            i`will receive an email detailing why your account cannot be closed and ` +
            i`any additional steps or information that may be required from you ` +
            i`before closing your account. `
          }
        />
      </p>
      ...
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`Wish will collect fees or other amounts from you, for your use of the ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`Services, as set forth in its policy on [Fees and Payments](${feesAndPaymentsLink}) ` +
            i`or as otherwise communicated to you by Wish. Except as set forth in ` +
            i`Paragraphs 15 and 16 herein, you are responsible for paying all fees ` +
            i`and applicable taxes associated with using and selling on Wish. `
          }
        />
      </p>
      <p>
      ...
      <p>
        ...
        policies, or other risks to Wish or third parties, or reflect pending or
        otherwise unrecoupable balances, then Wish may in its sole discretion
        withhold any payments to you for as long as Wish determines such
        conditions or any related risks to Wish or third parties persist. For
        any amounts that we determine you owe us, we may (a) charge your account
        or any payment instrument you provide to us; (b) offset any amounts that
      ...
      <h3>Appointment of Wish as Limited Payment Collection Agent</h3>
      <p>
        Merchants that receive payment from Wish as a result of listing items
        through or in connection with the Wish Services hereby appoint Wish as
        the merchants’ payment collection agent solely for the limited purpose
        of accepting funds from Wish users who purchase items through the Wish
        Services.
      </p>
      <p>
        As such, you agree that any funds, resulting from a Wish user’s purchase
        of items through the Wish Services and received by Wish from a Wish
        user, shall be considered the same as a payment made directly to you by
        a Wish user. You further agree that you will provide the purchased items
        to the Wish user in the agreed-upon manner as if you have received the
        payment directly from the Wish user. You agree that Wish may refund the
        Wish user in accordance with the Wish Return Policy. You understand that
        Wish’s obligation to pay you is subject to and conditioned upon
        successful receipt of the associated funds from Wish users who purchase
        items through the Wish Services. Wish guarantees payments to you only
        for such amounts that have been successfully received by Wish from Wish
        users in accordance with this Agreement and the Wish’s Terms of Use. In
        accepting appointment as your limited payment collection agent, Wish
        assumes no liability for any of your acts or omissions (including,
        without limitation, any violation by you of this Agreement).
      </p>
      <p>
        You agree that any obligation of a Wish user to pay you for the purchase
        of items through the Wish Services is extinguished upon the Wish user’s
        payment of the funds to Wish, upon which Wish then is responsible for
        remitting the funds to you in the manner described in this Agreement,
        the Payment Policy, or as otherwise communicated by Wish to you. Your
        funds are not eligible for payment to you unless and until all terms in
        this Agreement and Wish’s Terms of Use are satisfied. In the event that
        Wish does not remit any such payment or funds to you, you will have
        recourse only against Wish and not the Wish user directly.
      </p>
      ...
      <p>
        <Markdown
          text={
            i`**Binding Sale:** All sales are binding. You are obligated ` +
            i`to ship the applicable order in a prompt manner after a sale is made ` +
            i`over the Services or you otherwise complete the transaction with the ` +
            i`applicable buyer. The cost arising from not completing orders in time ` +
            i`shall be undertaken by you.`
          }
        />
      </p>
      <p>
      ...
      <p>
        ...
        <Markdown
          openLinksInNewTab
          text={
            i`Wish product listings, including those resulting from user searches, ` +
            i`are ranked according to various factors. These factors are intended to ` +
            i`help ensure that Wish customers see the items that they might find ` +
            i`most relevant, and include a Wish customer’s prior engagement with ` +
            i`Wish, a Wish customer’s prior orders and search history, the terms ` +
            i`used in a Wish customer’s search query, Wish’s attempts to expand and ` +
            i`understand the appeal or interest in new product categories or ` +
            i`offerings, the Wish customer’s location, the Wish merchant’s rating, ` +
            i`the historical revenue per impression of the product listing, the Wish ` +
            i`merchant’s refund rate, whether a Wish merchant participates in the ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`paid [ProductBoost](${productBoostLink}) ` +
            i`service (which participation might increase the ranking of a ` +
            i`merchant’s listing), and the overall standing of the Wish merchant on ` +
            i`the Wish platform. `
          }
        />
      </p>
      <p>
        Wish generally aims for equal treatment of the Wish merchants. However,
        Wish might differentiate merchants for the following reasons: (i)
        products offered by Wish merchants that participate in the paid
        ProductBoost service might be privileged due to economic and commercial
        reasons; (ii) products offered by merchants with high ratings may be
        favored over those merchants with lower ratings; (iii) products offered
        by merchants with low refund rates may be favored over those merchants
        with higher refund rates; (iv) products offered in regions in close
        proximity to a Wish customer may be favored over products offered
        outside those regions; (v) products offered by Wish or its affiliated
        companies might be privileged for economic and commercial reasons; and
        (vi) products whose merchants offer faster shipping options may be
        favored over products whose merchants offer slower shipping options.
        Privileged treatment may include higher position in rankings; increased
        direct or indirect remuneration to the merchant for use of the Services;
        or increased ability to offer products to specific users.
      </p>
      ...
      <ol type="a">
        ...
        <li>
          <Markdown
            openLinksInNewTab
            text={
              i`List any item on Wish (or consummate any transaction), link directly ` +
              i`or indirectly to, reference or contain descriptions of goods or ` +
              i`services that (i) are prohibited under these Terms, Wish’s ` + // eslint-disable-next-line local-rules/no-links-in-i18n
              i`[Terms of Use](${termsOfUseLink}), the [Merchant Policies](${merchantPolicyLink}), ` +
              i`or are prohibited in any other policy documents as posted by Wish; ` +
              i`(ii) are prohibited in any of the countries in which the items are ` +
              i`offered for sale; or (iii) could cause Wish to violate any ` +
              i`applicable law, statute, ordinance or regulation, or that violates ` +
              i`this Terms or any document incorporated therein; `
            }
          />
        </li>
        <li>
      ...
      <p>
        <strong>Business Data, Personal Data:</strong>
        &nbsp;
        <span>
          Wish collects and generates a variety of data in order to provide,
          market, and improve the Services.
        </span>
      </p>
      <p>
        Business Data: Wish collects, generates, and stores a variety of data
        that is not personal data (“Business Data”). Business Data can include
        information about: merchants and their stores; merchant accounts;
        merchant activity, including fulfillment data (e.g., tracking data on
        packages), compliance with Wish policies or participation in Wish
        programs such as ProductBoost or those related to fulfillment, and
        Content; user interest in products described by listings, including
        purchases, as well as user satisfaction with products and the purchase
        experience (e.g., customer reviews and refunds); and Merchant services
        providers such as enterprise resource planning providers (“ERPs”) or
        payment services providers (“PSPs”).
      </p>
      <p>
        Wish can access most Business Data and may permit its service providers
        to access certain Business Data to help provide the Services. Wish may
        share Business Data (including allowing access through technical means
        such as APIs) as instructed by a merchant, such as with merchant PSPs or
        ERPs. Merchants can access detailed information regarding their
        accounts, stores and activity by logging into their Wish accounts or
        their accounts with applicable service providers such as a carrier or
        PSP. Merchants can access data regarding the stores or listings of other
        Merchants by registering a user account and navigating to stores and
        listings of interest.
      </p>
      <p>
        Personal Data: Wish collects, generates, and stores a variety of
        personal data from individuals, including merchants and users. The Wish
        Privacy Policy describes in detail the personal data collected, the uses
        of such data, the manner in which it may be shared, and the choices
        individuals have about those personal data processing activities.
      </p>
      <p>
        <strong>Protection of User Dataa:</strong>
        &nbsp;
        <span>
          When you use the Services, such as when you fulfill a purchase, you
          may obtain personal information from or about a Wish user (“User
          Data”). Your use of User Data shall comply with applicable data
          protection law, including without limitation Europe’s General Data
          Protection Regulation. Unless you obtain a valid consent from the
          individuals described by User Data, you shall only use User Data in
          connection with the corresponding transaction with such user (e.g.
          shipping and fulfillment) or as necessary to meet your statutory legal
          requirements, such as tax and reporting requirements. You shall employ
          reasonable and appropriate measures to safeguard User Data from
          misuse, loss, destruction or unauthorized access or use. You
          acknowledge and agree that if Wish determines in good faith that
          additional agreements are necessary for compliance with applicable
          data protection law, you will promptly review and accept such
          agreements or cease using the Services or applicable portions thereof,
          such as sales into the European Union.
        </span>
      </p>
      <p>
      ...
      <p>
        <strong>
          Your Personal Data & Business Data – Legal Requirements Protection of
          Wish & Others:
        </strong>
        &nbsp;
        <span>
          You acknowledge and agree that your own personal data will be
          collected and used as described in Wish's Privacy Policy. Wish
          reserves the right to access, read, preserve, and disclose any
          Business Data (including Content) or other information that Wish in
          good faith believes is necessary to comply with law or court order;
          respond to legal, regulatory, or commercial claims; enforce or apply
          Wish’s policies, guidelines or other agreements; or protect the
          rights, property, or safety of Wish, its employees, users, or others.
          In connection with your use of the Services, and subject to the above,
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`event of a dispute, you and Wish each agree to first provide the other ` +
            i`a written notice (“Notice of Dispute”), which shall contain: (a) a ` +
            i`written description of the problem and relevant documents and ` +
            i`supporting information; (b) a statement of the specific relief sought; ` +
            i`and (c) the contact information of the party giving it. A Notice of ` +
            i`Dispute must be sent to: One Sansome Street, San Francisco, CA 94104 ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`or emailed at [merchant_support@wish.com](${merchantSupportEmail}). ` +
            i`Wish will provide a Notice of Dispute to you via the email address ` +
            i`associated with your Wish User ID, Merchant ID, or other information ` +
            i`provided to Wish by you. `
          }
        />
      </p>
      <p>
      ...
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`To begin an arbitration proceeding, you must send a letter requesting ` +
            i`arbitration and describing your claim to General Counsel, ContextLogic ` +
            i`Inc., One Sansome Street, 33rd Fl, San Francisco, CA 94104. The ` +
            i`arbitration will be conducted by a single arbitrator. Disputes ` +
            i`involving claims and counterclaims with an amount in controversy under ` + // eslint-disable-next-line local-rules/use-formatCurrency
            i`$250,000, not inclusive of attorneys' fees and interest, shall be ` +
            i`subject to JAMS' most current version of the Streamlined Arbitration ` +
            i`Rules and procedures available at ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`[http://www.jamsadr.com/rules-streamlined-arbitration/ ](${jamsadrRulesSlLink}); ` +
            i`all other claims shall be subject to JAMS's most current version of ` +
            i`the Comprehensive Arbitration Rules and Procedures, available at ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`[http://www.jamsadr.com/rules-comprehensive-arbitration/ ](${jamsadrRulesCompLink}). ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`JAMS's rules are also available at [www.jamsadr.com](${jamsadrLink}) ` +
            i`or by calling JAMS at 800-352-5267. If JAMS is not available to ` +
            i`arbitrate, the parties will select an alternative arbitral forum. If ` +
            i`there is a conflict between the JAMS Rules (or the rules of the ` +
            i`alternative arbitral forum selected by the parties) and the rules set ` +
            i`forth in this Agreement, the rules set forth in this Agreement will ` +
            i`govern. `
          }
        />
        &nbsp;
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`jurisdiction and venue over you if Wish otherwise qualifies for such ` +
            i`small claims court and the claim does not include a request for any ` +
            i`type of equitable relief. However, if you decide to pursue a claim in ` +
            i`small claims court, you agree to still provide Wish with advance ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`notice by email to [merchant_support@wish.com](${merchantSupportEmail}) ` +
            i`and by U.S. Mail to General Counsel, ContextLogic Inc., One Sansome ` +
            i`Street, 33rd Fl, San Francisco, CA 94104. `
          }
        />
      </p>
      <p>
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`Agreement by sending a timely written notice of your decision to opt ` +
            i`out to the following address: General Counsel, ContextLogic Inc., One ` +
            i`Sansome Street, 33rd Fl, San Francisco, CA 94104, or by email to ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`[merchant_support@wish.com](${merchantSupportEmail}), ` +
            i`within 30 days after first becoming subject to this Arbitration ` +
            i`Agreement. Your notice must include your name and address, your Wish ` +
            i`User ID (if any), Wish Merchant ID, the email address you used to set ` +
            i`up your Wish account (if you have one), and a clear statement that you ` +
            i`want to opt out of this Arbitration Agreement. If you opt out of this ` +
            i`Arbitration Agreement, all other parts of this Agreement will continue ` +
            i`to apply to you. Opting out of this Arbitration Agreement has no ` +
            i`effect on any other arbitration agreements that you may currently have ` +
            i`with us, or may enter into in the future with us. `
          }
        />
      </p>
      ...
      <h4>If you are an EU-Merchant, the following terms also apply:</h4>
      <p>
        <Markdown
          openLinksInNewTab
          text={
            i`Internal Complaint-handling System: We have an internal system for ` +
            i`handling your complaints free of charge, which you may use by ` +
            i`following the Initial Dispute Resolution procedure outlined above ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`(including by contacting [merchant_support@wish.com](${merchantSupportEmail})) ` +
            i`for the following issues: (i) alleged non-compliance by us with any ` +
            i`obligations laid down in the EU-Regulation 2019/1150 of 20 June 2019 ` +
            i`on promoting fairness and transparency for business users of online ` +
            i`intermediation services (“P2B-Regulation”) that you allege affects ` +
            i`you; (ii) technological issues that relate directly to the provision ` +
            i`of online intermediation services that you allege affect you; and ` +
            i`(iii) measures taken by, or behavior of, Wish that relate directly to ` +
            i`the provision online intermediation services that you allege affect ` +
            i`you. We will handle your Notices of Dispute within a reasonable time ` +
            i`frame and via the medium chosen by you when contacting us. `
          }
        />
      </p>
      <p>
        Mediation: In order to reach an out-of-court settlement of any disputes
        between you and Wish, including disputes that could not be resolved by
        the means of the Initial Dispute Resolution procedure outline above,
        Wish will cooperate with you in the selection of two European Union
        mediators reasonably agreeable to you and Wish. Any attempt to reach an
        agreement through mediation on the settlement of a dispute in accordance
        with these provisions shall not affect our right or your right to
        initiate judicial proceedings at any time before, during or after the
        mediation process. If requested by you, before entering into or during
        mediation, we shall make available to you information on the functioning
        and effectiveness of mediation related to our activities.
      </p>
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`proprietary or is licensed to Wish by Wish's merchants or ` +
            i`third-parties. You agree that you will not use any robot, spider, ` +
            i`scraper or other automated means to access Wish for any purpose ` +
            i`whatsoever, except to the extent expressly permitted by and in ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`compliance with these Terms and [Wish's API Terms of Service](${apiTOSLink}), ` +
            i`without Wish's prior express written permission. Additionally, you ` +
            i`agree that you will not: `
          }
        />
      </p>
      <ul>
      ...
      <p>
        ...
        any of your personal information or Content; Wish believes that you are
        acting inconsistently with the letter or spirit of Wish's policies, have
        engaged in improper or fraudulent activity in connection with Wish, or
        your actions may cause legal liability or financial loss to Wish or
        other merchants using the Services; your account remains unconfirmed;
        your account remains inactive for a period of one-hundred-eight (180)
        days; Wish determines that your account has been used to engage in
        deceptive, fraudulent, or illegal activity, or to substantially violate
        our policies or this Agreement; Wish determines that the performance of
        your obligations under this Agreement may result in returns, claims,
        disputes, violations of our terms or policies, or cause any other risks
        to Wish, its users or other third parties; or Wish determines, suspect,
        or is informed that you are selling goods or engaging in acts in
        violation of the prohibited activities defined below under Section 5 of
        this Agreement (including, without limitation, selling goods that are
        counterfeit, illegal, or violate third-party rights).
      </p>
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`regulatory fees, levies (specifically including environmental levies), ` +
            i`or charges and duties assessed, incurred, or required to be collected ` +
            i`or paid for any reason in connection with your use of the Services, ` +
            i`any advertisement, offer or sale of products, services or Content by ` +
            i`you on or through or in connection with the Services. This defined ` +
            i`term also means any of the types of taxes, duties, levies, or fees ` +
            i`mentioned above that are imposed on or collectible by Wish or any of ` +
            i`its Affiliates in connection with or as a result of fulfillment ` +
            i`services including the storage of inventory or packaging of products, ` +
            i`services or Content and other materials owned by you and stored by ` +
            i`Wish, shipping, or other actions by Wish. “Your Taxes,” however, does ` +
            i`not include any taxes collected and remitted by Wish as disclosed in ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`the [Tax Policy](${taxPolicyLink}). `
          }
        />
      </p>
      <h2>14. Insurance</h2>
      ...
      <p>
        <Markdown
          ...
          text={
            ...
            i`payment among all parties of this agreement, unless Wish chooses to ` + // eslint-disable-next-line local-rules/no-links-in-i18n
            i`collect and remit tax as disclosed in its [Tax Policy](${taxPolicyLink}). `
          }
        />
      </p>
      ...
      <p>
        ...
        us, any limitations on our liability, any confidentiality obligations,
        any terms regarding Wish’s ownership or intellectual property rights or
        any terms regarding disputes between us. The failure of either you or us
