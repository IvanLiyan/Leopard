...
import { silverMerchantStanding } from "@assets/illustrations";
...
export type SilverMerchantStandingModalProps = BaseProps;

@observer
class SilverMerchantStandingModalContent extends Component<
  SilverMerchantStandingModalProps
> {
  @computed
  get styles() {
    ...
    return (
      <div className={css(this.styles.root)}>
        <img
          ...
          src={silverMerchantStanding}
        />
        <div className={css(this.styles.topSection)}>
          <div className={css(this.styles.currentStanding)}>Silver</div>
          <div className={css(this.styles.explanation)}>
            Your store has demonstrated good customer experience performance and
            policy compliance.
          </div>
        </div>
          ...
          <ul className={css(this.styles.list)}>
            <li className={css(this.styles.listItem)}>
              {locale == "zh"
                ...
                : i`Your store’s newly listed and existing products ` +
                  i`are receiving complimentary impressions.`}
            </li>
            <li className={css(this.styles.listItem)}>
              <Markdown
                text={
                  locale == "zh"
                    ? "您店铺内的产品每月最多可获得**100万**个流量。"
                    : i`Your store’s products are eligible for up to **1M** ` +
                      i`additional impressions per month.`
                }
              />
            </li>
            <li className={css(this.styles.listItem)}>
...
export default class SilverMerchantStandingModal extends Modal {
  ...
  constructor(props: SilverMerchantStandingModalProps) {
    super((onClose) => <SilverMerchantStandingModalContent {...props} />);
    this.setHeader({
