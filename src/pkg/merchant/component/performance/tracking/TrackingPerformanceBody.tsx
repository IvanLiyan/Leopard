...
type TrackingPerformanceBodyProps = BaseProps & {
  readonly performanceData: PerformanceData;
};

const TrackingPerformanceBody = (props: TrackingPerformanceBodyProps) => {
  ...
  return (
    <>
      <div className={css(styles.dataContainer)}>
        ...
        <div className={css(styles.thresholdValue)}>{`> 95%`}</div>
      </div>
      <div className={css(styles.dataContainer)}>
        <div className={css(styles.policyLabel)}>Valid Tracking Rate</div>
        <div className={css(styles.thresholdLabels)}>Warning</div>
        <div className={css(styles.thresholdValue)}>{`< 80%`}</div>
      </div>
    </>
  );
};

export default TrackingPerformanceBody;
