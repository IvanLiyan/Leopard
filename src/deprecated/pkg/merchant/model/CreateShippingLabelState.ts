...
export class ShippingState {
  ...
  cheapestShippingOptionFrom(
    ...
    return minBy(options, ({ price: { amount } }) => amount);
  }

  fastestShippingOptionFrom(
    options: ReadonlyArray<PickedWpsShippingOption>
  ): PickedWpsShippingOption | undefined {
    return minBy(options, "daysToDeliver");
  }
  ...
  otherShippingOptionsFrom(
    ...
    const others = differenceBy(options, excluded, "id");
    return sortBy(others, ({ price: amount }) => amount);
  }
...
export default class CreateShippingLabelState {
  ...
  constructor({
    ...
    this.variationCount = product ? product.variationCount : 0;
  }
  ...
  get canBuyShippingLabel(): boolean {
    ...
    const selectedShippingOption = find(shippingOptions, [
      "id",
  ...
  onBuyShippingLabel = async (): Promise<boolean> => {
    ...
    if (isEdit) {
      ...
      toastStore.positive(i`Your shipping label has been updated`, {
        deferred: true,
      });
