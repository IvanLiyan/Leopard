...
const UploadVideo: React.FC<Props> = (props: Props) => {
  ...
  // setup taken from https://github.com/vercel/next.js/blob/canary/examples/with-web-worker/pages/index.js
  const cropWorkerRef = useRef<Worker>();
  useEffect(() => {
    cropWorkerRef.current = new Worker("../../../../../workers/crop-worker.ts");
  ...
  const runCropping = (
    ...
    return new Promise((accept, reject) => {
      ...
      if (cropWorkerRef.current) {
        cropWorkerRef.current.postMessage({
          type: "start",
          message: {
        ...
        cropWorkerRef.current.onmessage = (event: any) => {
          if (event.data.type === "progress") {
            ...
            const onUploadSuccess = (resp: { data: { image_url: string } }) => {
