...
import { MerchantAddressState } from "@merchant/model/tax/TaxIdentificationState";
...
import {
  ...
  TAX_ID_MASKS_BY_COUNTRY,
  TAX_ID_MASKS_BY_STATE,
...
const getGbMpfRows = ({
  ...
  const defaultShipFromLocation: CountryCode | undefined =
    shipping?.originCountryCode as CountryCode;
...
export const useCurrentCountry = (): CountryCode | null => {
  ...
  const { countryCode } = navigationStore.pathParams(
...
export const useCountryOptions = () =>
  useMemo((): ReadonlyArray<CountryType> => {
    ...
    let remainingCountryCodes: LegoCountryCode[] = (
      Object.keys(CountryNames) as ReadonlyArray<LegoCountryCode>
    ).filter((cc) => !topCountries.includes(cc));

    remainingCountryCodes = sortBy(
...
/**
 * Check whether text input is empty.
 */
export const isEmptyTextInput = (input: string | null | undefined): boolean => {
...
/** Check whether the given string input contains only numbers */
export const isNumber = (input: string): boolean => {
  return /^\d+$/.test(input);
};

/** Check whether the given string input is a valid tax id */
export const isValidTaxID = (
  id: string,
  locale: { countryCode: CountryCode; state?: string | null | undefined }
): boolean => {
  const { countryCode, state } = locale;

  let pattern;
  if (
    countryCode === "CA" &&
    state != null &&
    TAX_ID_MASKS_BY_STATE[countryCode][state] !== undefined
  ) {
    pattern = TAX_ID_MASKS_BY_STATE[countryCode][state];
  } else {
    pattern = TAX_ID_MASKS_BY_COUNTRY[countryCode];
  }

  // If mask exists, return validation result. Ow, treat the id as valid
...
/** Compute error based on validity of the given text input */
export const computeEmptyTextInputError = (
  input: string | null | undefined
): string | null => {
  if (isEmptyTextInput(input)) {
    return TAX_IDENTIFICATION_ERRORS.Required;
  }
  return null;
};

/** Return formatted address input */
export const formattedAddressInput = (
  address: MerchantAddressState,
  name: string | null | undefined
): AddressInput | null => {
  const { streetAddress1, streetAddress2, city, state, countryCode, zipcode } =
    address;

  if (name == null || streetAddress1 == null || city == null || state == null) {
    return null;
  }
